...
// Mendefinisikan tanggal dan waktu saat ini
var currentDate = new Date();

// Menetapkan tanggal target (1 Maret)
var targetDate = new Date(currentDate.getFullYear(), 2, 1); // 2 merupakan indeks bulan Maret

// Menambahkan 1 tahun jika tanggal target sudah lewat dari tanggal saat ini
if (currentDate.getMonth() > 2 || (currentDate.getMonth() === 2 && currentDate.getDate() > 1)) {
    targetDate.setFullYear(targetDate.getFullYear() + 1);
}

// Menghitung selisih waktu antara tanggal saat ini dan tanggal target
...
// Menampilkan hasil perhitungan mundur di elemen HTML yang diinginkan
document.querySelector('.circles').innerHTML = `
    <div class="circle">${days}<span>Days</span></div>
    <div class="circle">${hours}<span>Hours</span></div>
    <div class="circle">${minutes}<span>Minutes</span></div>
    <div class="circle">${seconds}<span>Seconds</span></div>
`;

// Update hitungan mundur setiap detik
setInterval(function() {
    // Mendefinisikan tanggal dan waktu saat ini lagi
    var currentDate = new Date();
    var timeDifference = targetDate.getTime() - currentDate.getTime();

    // Menghitung jumlah hari, jam, menit, dan detik lagi
    var days = Math.floor(timeDifference / (1000 * 60 * 60 * 24));
    var hours = Math.floor((timeDifference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    var minutes = Math.floor((timeDifference % (1000 * 60 * 60)) / (1000 * 60));
    var seconds = Math.floor((timeDifference % (1000 * 60)) / 1000);

    // Menampilkan hasil perhitungan mundur yang diperbarui di elemen HTML yang diinginkan
    document.querySelector('.circles').innerHTML = `
    <div class="circle">${days}<span>Days</span></div>
    <div class="circle">${hours}<span>Hours</span></div>
    <div class="circle">${minutes}<span>Minutes</span></div>
